...
bool Flag_RightMotor_MoveBackward=false;
bool Flag_LeftMotor_MoveBackward=false;
...
void moveBackward(){
	...
	Flag_RightMotor_MoveBackward = true;
	Flag_LeftMotor_MoveBackward = true;
  ...
  /* USER CODE BEGIN 5 */
	...
	for(;;)
	{
		...
		/*if(!HAL_GPIO_ReadPin(Key_GPIO_Port, Key_Pin)){
			...
			HAL_Delay(500);
			moveBackward();
			while(Flag_LeftMotor_MoveBackward || Flag_RightMotor_MoveBackward){
				HAL_GPIO_TogglePin(Led_GPIO_Port, Led_Pin);
			};
			HAL_Delay(500);
			moveFoward();
